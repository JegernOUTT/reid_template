...
from person_reid.data.base_transforms import jpg_decoder, jpeg_decoder, png_decoder, make_resize
from person_reid.data.face.ds_info_extractors import DATASETS_EXTRACTORS, IGNORE_VALUE, extract_ds_metadata
...
class FaceDataModule(pl.LightningDataModule):
    ...
    def _build_ds(self, paths, add_extra_filters=False, add_sampler_preprocessing=False,
        ...
        def _filter(dataset):
            if add_extra_filters:
                return dataset.select(lambda x: 'jpg' in x or 'png' in x or 'jpeg' in x)
            else:
                return dataset
    ...
    def _format_output(self, mode, data):
        output = {'image': data['image'],
                  ...
                  'with_mask': np.array(data['_with_mask'], dtype=np.long)}
    ...
    def _make_dataloader(self, mode):
        if mode == 'train':
            ...
            dataset = (self._build_ds(self._train_paths, True, True, shardshuffle=True)
                       ...
                       .map(make_resize(self._image_size))
    ...
    def _try_to_load_cache(self):
        ...
        cache_path = Path(self._train_paths[0]).parent / FaceDataModule.CACHE_FILENAME
...
if __name__ == '__main__':
    ...
    batch_size = 256
    data = FaceDataModule(train_paths=[p for p in (base_path / 'train/webface42m').iterdir() if p.suffix == '.tar'],
                          val_paths=[p for p in (base_path / 'test/glint360k').iterdir() if p.suffix == '.tar'],
                          sampler=dict(type='FaceClassSampler', shards_max_count=2000,
                                       output_path=base_path / 'train_shards'),
                          full_resample=False,
                          batch_shuffle=False,
    ...
    for item in tqdm(train_data):
        for i in range(batch_size):
            ...
            print(f'Is mask: {item["with_mask"][i].item()}\n\n')
            cv2.waitKey(0)
